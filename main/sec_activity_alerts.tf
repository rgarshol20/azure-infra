# Activity Log Alerts — satisfies Defender recommendation:
#   "An activity log alert should exist for specific Administrative operations"
# Covers CIS Azure Benchmark 5.1.x (NSG + security solution + policy operations)
...
resource "azurerm_monitor_activity_log_alert" "nsg_write" {
  ...
  scopes              = ["/subscriptions/${var.main_subscription_id}"]
  enabled             = true
