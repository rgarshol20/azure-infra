...
moved {
  from = azurerm_backup_policy_vm.acme_health_dashboard_v2
  to   = module.recovery_vault.azurerm_backup_policy_vm.vm_policies["acme_health_dashboard_v2"]
}

# Diagnostic settings for Recovery Services Vault (HIPAA audit trail)
module "diag_rsv_dashboard" {
  source = "../modules/diagnostic-settings"

  diagnostic_setting_name    = "diag-rsv-dashboard"
