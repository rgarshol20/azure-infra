# CIS Azure Benchmark 5.1.x — Activity Log Alerts (Logging subscription)
# Satisfies Defender recommendations for specific Administrative, Security, and Policy operations.
# Action group is defined in sec_alerts.tf (azurerm_monitor_action_group.critical_security).
...
resource "azurerm_monitor_activity_log_alert" "nsg_write" {
  ...
  scopes              = ["/subscriptions/${var.logging_subscription_id}"]
  enabled             = true
